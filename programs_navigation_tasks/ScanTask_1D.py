...
stimuliList = []
this_session = expInfo['session']
material = rf'materials/{this_session}List.xlsx'
material_list = data.importConditions(material)
dim_list = ['D1', 'D2', 'D3', 'D4']
shuffle(dim_list)
use_dim = dim_list[0]
unuse_dim = dim_list[1:len(dim_list)]
for i in range(len(material_list)):
    if material_list[i][unuse_dim[0]] == 1 and material_list[i][unuse_dim[1]] == 1 and material_list[i][
        unuse_dim[2]] == 1:
        stimuliList.append(rf"materials/{this_session}/{material_list[i]['file']}.png")

intro = mkImg(win, r'materials\intro_imgs\scan_D1.png')
...
thisExp.addData('dimension', use_dim)
# ----------------------trials------------------------
coin = 0
...
for thisRun in range(nRoute):
    ...
    core.wait(thisIPI)
    file_list, cur, goal = initialise_D1(space_shape, stimuliList, ['1', '4'])
    cur_file, goal_file, choiceF_file, choiceJ_file, corr_resp = file_list
    choice_list = ['1', '4', 'escape']
    RouteContinue = True
    thisBreak = False
    thisExp.addData('No.Path', thisRun + 1)
    ...
    while RouteContinue:
        ...
        if cur == goal:
            ...
            thisExp.addData('PathEnd', timeEndRoute)
        else:
            if choice != corr_resp:
            ...
            file_list, choice_list = updateRoute_D1(file_list, cur, goal, stimuliList, ['1', '4', 'escape'])
            cur_file, goal_file, choiceF_file, choiceJ_file, corr_resp = file_list
